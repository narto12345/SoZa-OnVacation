document.addEventListener("DOMContentLoaded", function () {
  ...
  const previewContainerPrincipal = document.getElementById(
    "preview-container-principal"
  );
  ...
  const previewContainerCarrusel = document.getElementById(
    "preview-container-carrusel"
  );
  ...
  function showImagePreviewCarrusel(files) {
    previewContainerCarrusel.innerHTML = ""; // Limpiar contenido previo
  ...
  fileInputCarrusel.addEventListener("change", function () {
    ...
    if (files.length > 0) {
      showImagePreviewCarrusel(files); // Manejar todas las imágenes seleccionadas
    }
  });
  ...
  dropZoneCarrusel.addEventListener("drop", function (event) {
    ...
    const files = event.dataTransfer.files;
    if (files.length > 0) {
      showImagePreviewCarrusel(files); // Manejar todas las imágenes arrastradas
    }
  });
