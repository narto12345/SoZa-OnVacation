...
def contact(request):
    ...
    if request.method == 'POST':
        form = ContactForm(request.POST)  # Se obtienen los datos del formulario y se guardan

        if form.is_valid():  # Esta parte es propia de django, lo usamos para que valide campos obligatorios y demás
...
def trazability(request):
    contacts = Contact.objects.all() ##Lo usamos para traer todos los datos del modelo y plasmarlos posteriormente en la tabla :)
